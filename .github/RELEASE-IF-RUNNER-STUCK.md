...
5. Upload the ZIP from the project root (e.g. `dw-product-catalog-1.5.6.zip` — filename is versioned; install folder is always `dw-product-catalog`).
6. Publish the release.

The plugin ZIP is the same whether created by Actions or by the local script: versioned filename, single root folder `dw-product-catalog/` so the install path is always `wp-content/plugins/dw-product-catalog/`.
